...
The input to the function is a list of file info data frames as they are 
returned by `kwb.fakin::read_file_info`.

```{r}
example_root <- system.file(package = "kwb.fakin")
```

```{r echo = FALSE}
# Create dummy folders to achieve least folder depth
dummy_folder <- file.path(example_root, "extdata/dummy/dummy")
kwb.utils::createDirectory(dummy_folder, dbg = FALSE)
writeLines("dummy", file.path(dummy_folder, "dummy_file.txt"))
```

```{r eval = FALSE}
file_info <- kwb.fakin::get_recursive_file_info(example_root)
info_matrix <- as.matrix(file_info)
kwb.fakin:::store(info_matrix, "working_with_paths.Rmd")
kwb.fakin::plot_all_treemaps(path_infos = file_info, as_png = FALSE)
```
By default the plots are saved to png files in the temporary directory. Here,
we plot into the current graphical device by setting `as_png` to `FALSE`.
...
Once you have split a vector of paths into its subfolder names you may use
the function `startsWithParts()` to filter for paths starting with a certain
sequence of subfolders:

```{r}
path_parts <- kwb.fakin:::splitPaths(paths)
start_parts <- c("project-1", "wp-1")

path_parts[kwb.fakin::startsWithParts(path_parts, start_parts)]
```

The function can also be used on a matrix of subfolders:

```{r}
folders <- kwb.fakin::toSubdirMatrix(paths)

folders[kwb.fakin::startsWithParts(folders, start_parts), ]
folders[kwb.fakin::startsWithParts(folders, c(start_parts, "input")), ]
```
